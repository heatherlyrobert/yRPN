...
yrpn_addr__init         (void)
{
   ...
   DEBUG_RPN    yLOG_schar   (s_ready);
   /*---(clear pointers)-----------------*/
   s_breaker   = NULL;
   DEBUG_RPN    yLOG_spoint  (s_breaker);
   s_maker     = NULL;
   DEBUG_RPN    yLOG_spoint  (s_maker);
   s_prettier  = NULL;
   DEBUG_RPN    yLOG_spoint  (s_prettier);
   s_adjuster  = NULL;
   DEBUG_RPN    yLOG_spoint  (s_adjuster);
   s_insider   = NULL;
   DEBUG_RPN    yLOG_spoint  (s_insider);
   /*---(update status)------------------*/
   s_ready     = 'i';
   DEBUG_RPN    yLOG_schar   (s_ready);
   /*---(complete)-----------------------*/
...
yRPN_addr_config        (void *a_breaker, void *a_maker, void *a_prettier, void *a_adjuster, void *a_insider)
{
   ...
   /*---(defense)------------------------*/
   DEBUG_RPN    yLOG_schar   (s_ready);
   DEBUG_RPN    yLOG_snote   (x_valid);
   --rce;  if (strchr ("iy", s_ready) == NULL) {
      DEBUG_RPN    yLOG_sexitr  (__FUNCTION__, rce);
      return rce;
   }
   /*---(save pointers)------------------*/
   s_breaker  = a_breaker;
...
yRPN__adj_check       (char *a_src, char a_scope, char *a_target)
{
   ...
   DEBUG_RPN    yLOG_complex ("target"    , "b=%4d, x=%4d, y=%4d, z=%4d", s_targb, s_targx, s_targy, s_targz);
   /*---(save)---------------------------*/
...
yRPN__adj_main     (cchar *a_src, cchar a_scope, cchar *a_target, cint b, cint x, cint y, cint z, char *a_out)
{
   ...
   /*---(defense)------------------------*/
   --rce;  if (s_ready != 'y') {
      DEBUG_RPN    yLOG_note    ("yRPN address handling functions not configured");
      DEBUG_RPN    yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
...
yRPN__badaddr        (int  a_pos)
{
   ...
   --rce;  if (zRPN_lang == YRPN_CBANG) {
      DEBUG_RPN     yLOG_note    ("skip in c lang mode");
      DEBUG_RPN     yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   ...
   --rce;  if (a_pos + 3 >= myRPN.l_working) {
      DEBUG_RPN    yLOG_note    ("not enough room for a bad address");
      DEBUG_RPN    yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   strlcpy (x_addr, myRPN.working + a_pos, 5);
   DEBUG_RPN    yLOG_info    ("x_addr"    , x_addr);
   --rce;  if (strcmp (x_addr, "#REF") != 0) {
      DEBUG_RPN    yLOG_note    ("does not match #REF");
      DEBUG_RPN    yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(handle)-------------------------*/
...
{  /*---(design notes)--------------------------------------------------------*/
   ...
   --rce;  if (rc < 0) {
      yRPN__token_error ();
      DEBUG_RPN     yLOG_note    ("address not valid");
      DEBUG_RPN     yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(handle)-------------------------*/
