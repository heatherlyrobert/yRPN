...
yRPN_normal        (char *a_src, char **a_rpn, int *a_nrpn, int a_max)
{
   ...
   strcpy (s_divtech, "\x1F");
   /*---(convert)------------------------*/
   rc = yRPN__driver (a_src, 'n', a_rpn, a_nrpn, a_max);
   DEBUG_YRPN    yLOG_value   ("driver"    , rc);
...
yRPN_detail        (char *a_src, char **a_rpn, int *a_nrpn, int a_max)
{
   ...
   strcpy (s_divtech, "\x1F");
   /*---(convert)------------------------*/
   rc = yRPN__driver (a_src, 'd', a_rpn, a_nrpn, a_max);
   DEBUG_YRPN    yLOG_value   ("driver"    , rc);
   DEBUG_YRPN    yLOG_value   ("pos"       , myRPN.pos);
   if (rc < 0) {
      DEBUG_YRPN    yLOG_exitr   (__FUNCTION__, rc);
...
yRPN_techtoken     (char *a_src, char **a_rpn, int *a_nrpn, int a_max)
{
   ...
   /*---(convert)------------------------*/
   rc = yRPN__driver (a_src, 't', a_rpn, a_nrpn, a_max);
   DEBUG_YRPN    yLOG_value   ("driver"    , rc);
   DEBUG_YRPN    yLOG_value   ("pos"       , myRPN.pos);
   if (rc < 0) {
      DEBUG_YRPN    yLOG_exitr   (__FUNCTION__, rc);
...
yRPN_parsed        (char *a_src, char **a_rpn, int *a_nrpn, int a_max)
{
   ...
   strcpy (s_divider, " ");
   /*---(convert)------------------------*/
   rc = yRPN__driver (a_src, 'p', a_rpn, a_nrpn, a_max);
   DEBUG_YRPN    yLOG_value   ("driver"    , rc);
   DEBUG_YRPN    yLOG_value   ("pos"       , myRPN.pos);
   if (rc < 0) {
      DEBUG_YRPN    yLOG_exitr   (__FUNCTION__, rc);
...
yRPN__convert       (char *a_source)
{
   ...
   while (myRPN.pos < myRPN.l_working) {
      ...
      if (rc <= myRPN.pos && strchr (YSTR_ADDR  , x_ch) != 0)  rc = yRPN__addresses  (myRPN.pos, s_ctab);
      if (rc <= myRPN.pos && strchr (YSTR_LOWER , x_ch) != 0)  rc = yRPN__keywords   (myRPN.pos);
      if (rc <= myRPN.pos && strchr (YSTR_LOWER , x_ch) != 0)  rc = yRPN__types      (myRPN.pos);
   ...
   --rce;  if (rc < 0) {
      ...
      strlcpy (myRPN.shuntd, YRPN_TOKEN_NULL, LEN_RECD);
      strlcpy (myRPN.tokens, YRPN_TOKEN_NULL, LEN_RECD);
      strlcpy (myRPN.normal, YRPN_TOKEN_NULL, LEN_RECD);
